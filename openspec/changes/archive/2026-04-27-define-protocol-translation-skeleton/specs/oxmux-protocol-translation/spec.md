...
The `oxmux` core SHALL expose canonical protocol request and response structures for future proxy handling without performing provider translation or network transport.
...
The `oxmux` core SHALL define translation interfaces that return structured errors or explicit placeholder results when translation behavior is intentionally deferred.
