...
Define the `oxmux` headless core model registry and listing contracts for configured model identifiers, provider-native targets, aliases, forks, provider/account applicability, routing eligibility, streaming support, disabled/degraded state, and future model-list route projections without live provider discovery.
## Requirements
